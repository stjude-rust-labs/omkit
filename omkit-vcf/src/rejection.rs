...
impl RejectionReason {
    /// Returns the VCF `FILTER` identifier for this rejection reason.
    ///
    /// The VCF specification uses the `FILTER` column (column 7) to
    /// indicate whether a record passed all quality filters. Records
    /// that pass carry `PASS` (or `.`), while records that fail carry
    /// one or more semicolon-delimited filter identifiers. Each
    /// identifier must be declared in a `##FILTER` header line with an
    /// `ID` and `Description` field (see
    /// [`description()`](Self::description)).
    ///
    /// When a record is rejected during transformation, the pipeline
    /// writes it to a rejected output file with the appropriate filter
    /// identifier in the `FILTER` column so downstream consumers can
    /// determine why the record was excluded.
