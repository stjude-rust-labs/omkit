...
use crate::record::extensions::Extension;
use crate::rejection::RejectionReason;

/// State produced by the [`RefValidator`] for downstream transformers.
#[derive(Clone, Debug)]
pub struct RefValidatorState {
    /// Whether a `REF`/`ALT` swap was detected.
    pub swap_detected: bool,
}

impl Extension for RefValidatorState {}
    ...
    R: BufRead + Seek + Send,
{
    fn transform(&self, mut record: VcfRecord) -> Result<Outcome> {
        let contig = record.current().reference_sequence_name();
        let Some(start) = record.current().variant_start() else {
        ...
        // NOTE: unlike `CoordinateMapper`, which rejects records with missing
        // `REF` (since it needs `REF` to build the liftover interval), this
        // transformer passes them through because there is nothing to validate.
        if ref_bases == MISSING || ref_bases.is_empty() {
            trace!(contig = %contig, position = start.get(), "REF is missing, skipping validation");
        ...
        let end = Position::try_from(start.get() + ref_len - 1).unwrap();
        let region = Region::new(contig, start..=end);

        let fasta_seq = {
            // SAFETY: we don't ever expect the lock to be poisoned.
            let mut reader = self.reader.lock().unwrap();
            match reader.query(&region) {
                // NOTE: ASCII uppercase is used because we don't care about the casing of the
                // nucleotides in the reference FASTA.
                Ok(record) => record.sequence().as_ref().to_ascii_uppercase(),
                Err(e) => {
                    trace!(
                        contig = %contig,
                        position = start.get(),
        ...
        // NOTE: ASCII uppercase is used because we don't care about the casing of the
        // nucleotides in the reference column of the variant.
        let req_seq = ref_bases.as_bytes().to_ascii_uppercase();

        if fasta_seq == req_seq {
            trace!(
                contig = %contig,
                position = start.get(),
        ...
        match &self.mode {
            ...
            Mode::DetectSwap => {
                // NOTE: this assumes the record is monoallelic. A
                ...
                assert!(
                    valid_alts.len() <= 1,
                    "`DetectSwap` requires monoallelic records; found {} `ALT` alleles at \
                     `{contig}`:`{start}`. Run `SplitMultiAllelic` upstream.",
                    valid_alts.len(),
                );
                ...
                if alt.as_bytes().to_ascii_uppercase() == fasta_seq {
                    ...
                    trace!(
                        ...
                        position = start.get(),
                        ref_allele = %ref_bases,
                        "ALT allele does not match reference either, no swap possible"
                    );
                    self.stats
                        .rejected_mismatched
                        .fetch_add(1, Ordering::Relaxed);
...
mod tests {
    ...
    fn detect_swap_accepts_matching_ref() {
        ...
        let outcome = validator.transform(record).unwrap();
        match outcome {
            Outcome::Accepted(r) => assert!(r.extensions().get::<RefValidatorState>().is_none()),
    ...
    #[test]
    #[should_panic(expected = "`DetectSwap` requires monoallelic records")]
